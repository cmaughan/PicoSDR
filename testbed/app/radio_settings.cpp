...
RadioSettings radio_settings_load_settings(const toml::table& settings)
{
    ...
    try
    {
        ...
        radioSettings.targetCenterHz = read_float("radio_target_center_hz", radioSettings.targetCenterHz);
...
toml::table radio_settings_save_settings(const RadioSettings& settings)
{
    ...
    tab.insert_or_assign("radio_target_center_hz", settings.targetCenterHz);
...
void radio_settings_validate_settings(RadioSettings& settings)
{
    ...
    settings.targetCenterHz = std::clamp(settings.targetCenterHz, 0.0f, 1000.0f);
